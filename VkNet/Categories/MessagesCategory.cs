...
namespace VkNet.Categories
{
    ...
    public class MessagesCategory
    {
            ...
            long? lastMessageId = null)
        {
            ...
            do
            {
                var currentItems = Get(type, out totalCount, count, i * count, timeOffset, filter, previewLength, lastMessageId);
            ...
            long? startMessageId = null)
        {
            ...
            do
            {
                var currentItems = GetHistory(id, isChat, out totalCount, i * count, count, inReverse, startMessageId);
                if (currentItems != null) result.AddRange(currentItems);
            } while (++i * count < (_vk.CountFromLastResponse ?? 0));
        ...
        public ReadOnlyCollection<Message> GetAllDialogs(out int totalCount, out int unreadCount, bool unread = false, long? userId = null, long? chatId = null, int? previewLength = null)
        {
            ...
            do
            {
                var currentItems = GetDialogs(count, i * count, out totalCount, out unreadCount, unread, userId, chatId, previewLength);
                if (currentItems != null) result.AddRange(currentItems);
            } while (++i * count < (_vk.CountFromLastResponse ?? 0));
        ...
        public ReadOnlyCollection<Message> SearchAll([NotNull] string query, out int totalCount)
        {
            ...
            do
            {
                var currentItems = Search(query, out totalCount, count, i * count);
                if (currentItems != null) result.AddRange(currentItems);
            } while (++i * count < (_vk.CountFromLastResponse ?? 0));
